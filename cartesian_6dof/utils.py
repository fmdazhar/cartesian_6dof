...
class LinearTargetInterpolator:
    """
    C¹-continuous blend from start → goal SE(3) in `duration` seconds.

    Call ``step(dt)`` every control cycle; check ``done`` when finished.
    """

    def __init__(self, start_T: "cartesian_6dof.SE3", goal_T: "cartesian_6dof.SE3", duration: float):
        self._start_T = start_T
        self._goal_T = goal_T
        self._duration = max(duration, 1e-6)
        self._elapsed = 0.0

    def step(self, dt: float) -> "cartesian_6dof.SE3":
        self._elapsed = min(self._elapsed + dt, self._duration)
        a = self._elapsed / self._duration
        pos = (1.0 - a) * self._start_T.translation + a * self._goal_T.translation
        quat = _slerp(self._start_T.rotation, self._goal_T.rotation, a)
        return cartesian_6dof.SE3(quaternion=quat, translation=pos)
