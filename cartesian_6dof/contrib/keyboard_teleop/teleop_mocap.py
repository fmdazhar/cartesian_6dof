...
class TeleopMocap:
    ...
    def __init__(self, data):
        ...
        self.actions = {
            ...
            keycodes.KEY_BACKSLASH: self.toggle_tracking,
            ...
            keycodes.KEY_O: self.open_gripper,    # O: open gripper
            keycodes.KEY_P: self.close_gripper,   # P: close gripper
            keycodes.KEY_G: self.set_new_goal,
        }
        self.movements = {
    ...
    def open_gripper(self):
        ...
        # Assuming `self.data.gripper` is in [0,1]
        self.data.ctrl[6] = 0.0
        print("Gripper opened.")

    def close_gripper(self):
        """Close the gripper fully."""
        self.data.ctrl[6] = 255.0
        print("Gripper closed.")

    # --- new mocap-tracking toggle ------------------------------------------
    def toggle_tracking(self):
        """Start/stop following the target with the mocap body."""
        self.tracking = not self.tracking
        state = "On" if self.tracking else "Off"
        print(f"Mocap tracking toggled: {state}!")

    def set_new_goal(self):
        """Raise a one-shot flag that the main loop can consume."""
        self.new_goal = True
        print("🔹 New goal requested.")
