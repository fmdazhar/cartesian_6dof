...
class SE3(MatrixLieGroup):
    ...
    def from_frame_name(
        ...
        """
        Get the world-space pose of an arbitrary MuJoCo frame (site/body/geom).

        Works on MuJoCo ≥2.2; uses *xquat if available, otherwise converts
        the 3×3 matrix (*xmat) to a quaternion with mju_mat2Quat.
        """
        quat = np.empty(4, dtype=np.float64)            # output buffer

        if frame_type == "site":
            fid  = model.site(frame_name).id
            pos  = data.site_xpos[fid]
            if hasattr(data, "site_xquat"):
                quat[:] = data.site_xquat[fid]
            else:                                       # pre-2.3 fallback
                mujoco.mju_mat2Quat(quat, data.site_xmat[fid])
        elif frame_type == "body":
            fid  = model.body(frame_name).id
            pos  = data.xpos[fid]
            quat[:] = data.xquat[fid]                   # bodies always have xquat
        elif frame_type == "geom":
            fid  = model.geom(frame_name).id
            pos  = data.geom_xpos[fid]
            if hasattr(data, "geom_xquat"):
                quat[:] = data.geom_xquat[fid]
            else:
                mujoco.mju_mat2Quat(quat, data.geom_xmat[fid])
        else:
            raise ValueError(f"Unsupported frame_type '{frame_type}'")

        return cls.from_rotation_and_translation(SO3(quat), pos.copy())
    ...
    @staticmethod
    def resolve_goal(spec, current_T: "SE3") -> Goal:
        """Turn a (pos, quat, grip, wait) spec into a fully-formed Goal.

        * `pos`  may be None → keep current position.
        * `quat` may be None → keep current orientation.
        * `grip` may be None → leave gripper unchanged.
        * `wait` may be None → default to 0.
        """
        pos, quat, grip, wait = spec

        # --- position --------------------------------------------------
        if pos is None:
            pos = current_T.translation()
        else:
            pos = np.asarray(pos, dtype=np.float64)

        # --- orientation ----------------------------------------------
        if quat is None:
            quat = current_T.rotation().wxyz
        else:
            quat = np.asarray(quat, dtype=np.float64)

        pose = SE3.from_rotation_and_translation(SO3(quat), pos)
        return Goal(pose=pose, gripper_cmd=grip, wait_steps=int(wait or 0))
