...
if __name__ == "__main__":
    ...
    # Convert those names to geom IDs
    arm_collision_geoms = [model.geom(name).id for name in _arm_collision_geom_names]

    _gripper_collision_geom_names = [
        "base_mount_col",   "base_gripper_col",
        "right_driver_col", "right_coupler_col",
        "right_spring_col", "right_follower_col",
        "right_pad1",       "right_pad2",
        "left_driver_col",  "left_coupler_col",
        "left_spring_col",  "left_follower_col",
        "left_pad1",        "left_pad2",
    ]
    gripper_collision_geoms = [model.geom(name).id for name in _gripper_collision_geom_names]
    ...
    with mujoco.viewer.launch_passive(
        ...
        mink.move_mocap_to_frame(model, data, "target", "pinch", "site")
        ...
        while viewer.is_running():
            ...
            if key_callback.tracking:
                ...
                goal_T = mink.SE3.from_mocap_name(model, data, "target")
