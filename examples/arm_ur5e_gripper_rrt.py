...
from cartesian_6dof.planning.rrt import RRTPlanner, RRTPlannerOptions # import the RRT planner
                ...
                if path is not None and len(path) > 1:
                    ...
                    if shortcut:
                        short_path = shortcut_path(model, path, geom_id_pairs, max_iters=200,
                                         max_step_size=2e-3)
                        print(f"[RRT] Shortcutted path to {len(short_path)} waypoints")
                        active_path = short_path
                    else:
                        active_path = path
                    active_path = discretize_joint_space_path(active_path, 2e-3)
                    print(f"[RRT] Discretized path to {len(active_path)} waypoints")
                    path_idx = 1
                else:
